...
class Line {
  ...
  reset = (x, y, init) => {
    if (
      init ||
      Math.abs(x) > (
        (this.width / 2) - (this.particleSize * this.segmentLen)
      ) ||
      Math.abs(y) > (
        (this.height / 2) - (this.particleSize * this.segmentLen)
      )
    ) {
      this.turningX = 0;
      this.turningY = 0;
  ...
  // 移动: 绘制线条
  step = () => {
    const x = this.turningX + (
      Math.cos(this.angle) * this.currentSegmentLen * this.particleSize
    );
    const y = this.turningY + (
      Math.sin(this.angle) * this.currentSegmentLen * this.particleSize
    );
  ...
  drawSpark = (x, y) => {
    if (Math.random() < this.sparkChance) {
      const rectX = x + (
        (Math.random() < 0.5 ? 1 : -1) * Math.random() * this.sparkDist
      );
      const rectY = y + (
        (Math.random() < 0.5 ? 1 : -1) * Math.random() * this.sparkDist
      );
